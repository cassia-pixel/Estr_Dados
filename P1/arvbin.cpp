...
//Chama função que escreve em ordem crescente
...
//Chama a função de busca a partir da raíz
...
No *ArvBinBusca::minimo(No *x) {
  //TODO: implementar
  return (x->esq != NULL) ? minimo(x->esq) : x; //Se x->esq não for nulo chama o mínimo para ele, senão, retorna o x
}
...
No *ArvBinBusca::maximo(No *x) {
  //TODO: implementar
  return (x->dir != NULL) ? maximo(x->dir) : x; //Se x->dir não for nulo chama o máximo para ele, senão, retorna o x
}
...
No *ArvBinBusca::sucessor(No *x) {
  ...
  if(x->dir != NULL)  //Verifica se tem algum nó na direita 
    return minimo(x->dir);  //Chama a função mínimo para o nó da direita
  
  aux = x->pai;

  while(aux != NULL && x == aux->dir) {
    x = aux; 
    aux = aux->pai;
  }
  return aux;
...
void ArvBinBusca::limpa(No *x) {
  //TODO: implementar
  if(x->esq != NULL) //Tem filho esquerdo
    limpa(x->esq);
  if(x->dir != NULL) //Tem filho direito
    limpa(x->dir);
  delete x;
} 
