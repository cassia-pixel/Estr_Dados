...
private:
  ...
  static void desloca_dir(T vetor[], int inicio, int fim); // os elementos [inicio,fim]  (fim é movido p/ fim+1)s
};
...
pair<No *, int> BArvore::busca(No *x, int k) {
  ...
  for(i = 0; i < x->n and k > x->chave[i]; ++i); //Percorre até chegar no final ou encontrar uma chave maior que k

  if(i < x->n and x->chave[i] == k) //Se achar a chave procurada
    return pair<No *, int>(x, i);
  else if(x->folha) //Caso não ache a chave
    return pair<No *, int>(NULL, -1);
  else 
    return busca(x->filho[i], k); //Se x não for folha e ainda der para procurar 
}
...
pair<No *, int> BArvore::minimo(No *x) {
  ...
  // neste nó.
  while(not x->folha) { //Enquanto x não for folha 
    x = x->filho[x->n - x->n];  //x recebe sempre o seu filho na extrema direita 
  }
  return pair<No *, int>(x, x->n - x->n);  //Retorna o primeiro filho em uma folha na extrema direita(mínimo)
}
...
pair<No *, int> BArvore::maximo(No *x) {
  ...
  // neste nó.
  while(not x->folha) { //Enquanto x não for folha 
    x = x->filho[x->n]; //x recebe sempre o seu filho na extrema esquerda 
  }
  return pair<No *, int>(x, x->n - 1);  //Retorna o último filho de uma folha na extrema esquerda(máximo)
}
...
void BArvore::limpa(No *x) {
  ...
  //fazer laço pra apagar os filhos e depois apagar o no em si
  int i;
  if(!x->folha) {
    for(i = 0; i <= x->n; ++i) 
      limpa(x->filho[i]);
